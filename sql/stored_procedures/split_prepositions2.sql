CREATE PROCEDURE `split_prepositions2`(IN that_td_id INT, IN this_te_id INT, IN that_df_uuid CHAR(36), 
...
BEGIN
       ...
       DECLARE that_explicit_spelling, new_explicit_spelling, prep_explicit_spelling, that_form, that_type VARCHAR(60) DEFAULT '';
		...
		SET FOREIGN_KEY_CHECKS=0;
			SELECT form INTO that_form FROM dictionary_form WHERE uuid = that_df_uuid;
			...
			CALL `oarebyue_0.3`.`iterate_obj_in_text_inclusive_insert`(that_uuid);
		...
		CALL `oarebyue_0.3`.`iterate_obj_in_text_inclusive_insert`(that_uuid);
			CALL `oarebyue_0.3`.`iterate_word_on_tablet_inclusive_insert`(that_uuid); 
			SET @prep_uuid := UUID();
			INSERT INTO text_discourse (`uuid`,`type`,`obj_in_text`,`word_on_tablet`,`child_num`,`text_uuid`,`tree_uuid`,`parent_uuid`,`spelling_uuid`,`explicit_spelling`,`transcription`) 
						VALUES (@prep_uuid, 'word', that_obj_in_text+1, that_word_on_tablet, '1', that_text_uuid, that_tree_uuid, @new_phrase_uuid, this_spell_uuid, this_explicit_spell, CONCAT(this_explicit_spell,'-'));
                  	-- UPDATE the original row to no longer have the preposition and update spelling_uuid
			UPDATE text_discourse SET parent_uuid = @new_phrase_uuid, child_num = '2', explicit_spelling = that_explicit_spell, spelling_uuid = that_spell_uuid, transcription = that_form WHERE id = that_td_id;
