DROP PROCEDURE `enphrase_3_phrases_w_prop`;
DELIMITER $$
...
BEGIN
       ...
       DECLARE this_type, that_type, final_type VARCHAR(60) DEFAULT '';
	...
	SELECT `type` INTO final_type FROM text_discourse WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;
	...
	SELECT obj_in_text INTO final_obj_in_text FROM text_discourse WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;
	...
	SELECT parent_uuid INTO final_parent_uuid FROM text_discourse WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;
	...
	SELECT text_uuid INTO final_text_uuid FROM text_discourse WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;
	...
	SELECT tree_uuid INTO final_tree_uuid FROM text_discourse WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;
    ...
    SELECT `child_num` INTO final_child_num FROM text_discourse WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;

	IF this_type = 'phrase'
		AND that_type = 'phrase'
		AND final_type = 'phrase'
		AND this_text_uuid = that_text_uuid
		AND this_text_uuid = final_text_uuid
		AND this_parent_uuid = that_parent_uuid
		AND this_parent_uuid = final_parent_uuid
		AND this_tree_uuid = that_tree_uuid
		AND this_tree_uuid = final_tree_uuid
	...
	THEN
		...
		UPDATE text_discourse SET parent_uuid = @new_phrase_uuid, child_num = '3' WHERE uuid = CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci;
				...
				WHERE reference_uuid IN (SELECT reference_uuid
					FROM (SELECT reference_uuid 
						FROM item_properties 
							...
							(CONVERT(this_uuid USING latin1) COLLATE latin1_swedish_ci, CONVERT(that_uuid USING latin1) COLLATE latin1_swedish_ci, CONVERT(final_uuid USING latin1) COLLATE latin1_swedish_ci) 
