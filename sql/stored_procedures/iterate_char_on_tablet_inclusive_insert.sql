CREATE PROCEDURE `iterate_char_on_tablet_inclusive_insert`(IN this_uuid CHAR(36))
	BEGIN
		...
		UPDATE text_epigraphy SET char_on_tablet = (char_on_tablet+1) WHERE text_uuid = CONVERT(this_text_uuid USING latin1) COLLATE latin1_swedish_ci AND parent_type = "line" AND char_on_tablet >= this_char_on_tablet ORDER BY object_on_tablet ASC;
    END