CREATE PROCEDURE `iterate_child_num_exclusive_insert`(IN this_uuid CHAR(36))
	BEGIN
		...
		UPDATE text_discourse SET child_num = (child_num+1) WHERE parent_uuid = CONVERT(this_parent_uuid USING latin1) COLLATE latin1_swedish_ci AND child_num >= (this_child_num+1) AND uuid != CONVERT(this_uuid USING latin1) COLLATE latin1_swedish_ci ORDER BY obj_in_text ASC;
    END