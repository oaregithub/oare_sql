DROP PROCEDURE iterate_object_on_tablet_after_insert;
DELIMITER //
CREATE PROCEDURE iterate_object_on_tablet_after_insert(IN this_uuid CHAR(36))
	BEGIN
		...
		UPDATE text_epigraphy SET object_on_tablet = (object_on_tablet+1) WHERE text_uuid = CONVERT(this_text_uuid USING latin1) COLLATE latin1_swedish_ci AND object_on_tablet >= (this_object_on_tablet+1) ORDER BY object_on_tablet ASC;
    END //
DELIMITER ;