CREATE PROCEDURE `iterate_char_on_line_inclusive_insert`(IN this_uuid CHAR(36))
	BEGIN
		...
		IF ((SELECT `type` FROM text_epigraphy WHERE parent_uuid = CONVERT(this_parent_uuid USING latin1) COLLATE latin1_swedish_ci) = 'line') THEN
